...
def pooled_features(
    spike_times: torch.Tensor, t_target: float | None = None, pool_size: int = 2
) -> torch.Tensor:
    ...
    decode→pool pipeline cannot drift between them.
    """
    from spikinn.evaluation.feature_extraction import spike_times_to_features

    return sum_pool_features(spike_times_to_features(spike_times, t_target=t_target), pool_size)
