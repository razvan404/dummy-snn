...
ORDERINGS = [
    ...
    # Training-log-based orderings (require last10k_winners from training_logs.pt)
    "training_early_spike",  # earliest spike time at end of training first
    "training_late_spike",  # latest spike time at end of training first
    "hybrid_training_spike",  # interleaved: earliest ↔ latest training spike time
    # Recent-win orderings (require last10k_winners from training_logs.pt)
    # Neurons that won most recently had their threshold just changed.
    "recent_winner",  # most recently changed threshold first
    "oldest_winner",  # least recently changed threshold first
    "hybrid_recent_winner",  # interleaved: most recent ↔ oldest
]
...
def get_filter_order(
    ...
    *,
    ...
    match ordering:
        ...
        case "recent_winner" | "oldest_winner" | "hybrid_recent_winner":
            if last_win_index is None:
                raise ValueError(
                    f"ordering={ordering!r} requires last_win_index "
                    "(needs last10k_winners in training_logs.pt)"
                )
            # Filters that never won (-1) go last
            never_won = np.where(last_win_index < 0)[0]
            won = np.where(last_win_index >= 0)[0]
            asc_recency = won[np.argsort(last_win_index[won])]
            if ordering == "recent_winner":
                return np.concatenate([asc_recency[::-1], never_won])
            elif ordering == "oldest_winner":
                return np.concatenate([asc_recency, never_won])
            else:
                return np.concatenate([_interleave(asc_recency[::-1]), never_won])
        case _:
