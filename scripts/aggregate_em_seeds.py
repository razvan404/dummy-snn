...
def main() -> None:
    ...
    for dataset in args.datasets:
        ...
        print(
            f"{'seed':>4}  "
            f"{'base_tr':>8} {'fin_tr':>8} {'Δtr_fin':>8}   "
            f"{'base_te':>8} {'fin_te':>8} {'Δte_fin':>8} {'best_te':>8} {'Δte_best':>9}"
        )
        for r in rows:
            print(
                f"{r['seed']:>4}  "
                f"{r['base_train']:>8.4f} {r['final_train']:>8.4f} {r['dtrain_final']:>+8.4f}   "
                f"{r['base_test']:>8.4f} {r['final_test']:>8.4f} {r['dtest_final']:>+8.4f} "
                f"{r['best_test']:>8.4f} {r['dtest_best']:>+9.4f}"
            )
        ...
        print(
            f"\n  mean Δtest (final): {agg['dtest_final']['mean']:+.4f} ± {agg['dtest_final']['std']:.4f}  "
            f"(n={len(rows)})"
        )
        print(
            f"  mean Δtest (best):  {agg['dtest_best']['mean']:+.4f} ± {agg['dtest_best']['std']:.4f}"
        )
        print(
            f"  mean Δtrain (final): {agg['dtrain_final']['mean']:+.4f} ± {agg['dtrain_final']['std']:.4f}"
        )
    ...
    for ax, dataset in zip(
        ...
        d_best = [r["dtest_best"] * 100 for r in rows]
        d_final = [r["dtest_final"] * 100 for r in rows]
        x = np.arange(len(seeds))
        w = 0.35
        ax.bar(x - w / 2, d_best, w, label="best iter", color="C2")
        ax.bar(x + w / 2, d_final, w, label="final iter", color="C0")
        ax.axhline(0, color="k", lw=0.8)
        ax.axhline(
            agg["dtest_best"]["mean"] * 100,
            color="C2",
            ls="--",
            alpha=0.6,
            label=f"mean best {agg['dtest_best']['mean'] * 100:+.2f} pp",
        )
        ax.set_xticks(x)
        ...
        ax.set_title(
            f"{dataset}  Δtest_best = {agg['dtest_best']['mean'] * 100:+.2f} ± {agg['dtest_best']['std'] * 100:.2f} pp"
        )
